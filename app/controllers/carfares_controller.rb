class CarfaresController < ApplicationController
  before_action :set_user, only: [:new, :new_1, :destroy, :update, :edit, :show, :show_1, :index, :update_show, :update_show_admin, :carfares_application]
  before_action :current_user, only: [:new, :new_1, :destroy, :update, :edit, :show, :show_1, :index, :update_show, :update_show_admin]
  ...
  def index_admin
    @carfares = Carfare.where.not(date_of_use: nil, application: nil).order(:date_of_use, :id).paginate(page: params[:page], per_page: 2)
  ...
  def index_admin_1
    @carfares = Carfare.where.not(date_of_use_private_car: nil, application_1: nil).order(:date_of_use_private_car, :id).paginate(page: params[:page], per_page: 2)
  ...
  # 交通費検索ページ（公共機関）
  def search
    @results = @q.result.where.not(date_of_use: nil).order(:date_of_use, :id).paginate(page: params[:page], per_page: 2)
  end
  ...
  # 交通費新規登録ページ（公共機関）
  def carfare_params
    params.require(:carfare).permit(:name, :date_of_use, :commuting_place, :point_of_departure, :public_transportation_arrival, :public_institution , :public_transportation_departure, :parking_fee, :public_transportation_cash, :hotel_charge, :moving_distance, :highway_rate, :image, :application)
  end
